-- AGRIVA server-side checkout/order creation foundation.
...
create or replace function public.create_order_from_cart(p_shipping_address jsonb,p_delivery_fee numeric default 0)
...
begin
 ...
 insert into public.orders(buyer_id,subtotal,delivery_fee,total,shipping_address) values(uid,total_sub,p_delivery_fee,total_sub+p_delivery_fee,p_shipping_address) returning * into o;
 ...
 insert into public.order_events(order_id,event_type,event_key,metadata) values(o.id,'created','order-created-'||o.id::text,jsonb_build_object('subtotal',total_sub)) on conflict(event_key) do nothing;
 return o;
end;$$;
grant execute on function public.create_order_from_cart(jsonb,numeric) to authenticated;
