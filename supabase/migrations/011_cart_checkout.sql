-- AGRIVA persistent cart and checkout foundation.
...
create or replace function public.add_cart_item(p_product_id uuid,p_quantity integer,p_unit_price numeric)
...
begin
 ...
 if p_quantity < 1 or p_quantity > 1000 then raise exception 'Invalid quantity'; end if;
 select * into c from public.carts where user_id=uid and status='active' for update;
 if not found then insert into public.carts(user_id) values(uid) returning * into c; end if;
 insert into public.cart_items(cart_id,product_id,quantity,unit_price) values(c.id,p_product_id,p_quantity,p_unit_price)
 on conflict(cart_id,product_id) do update set quantity=least(1000,public.cart_items.quantity+excluded.quantity),unit_price=excluded.unit_price,updated_at=now()
 returning * into i;
...
declare uid uuid; c uuid; begin uid:=auth.uid(); if uid is null then raise exception 'Authentication required'; end if; select id into c from public.carts where user_id=uid and status='active'; if c is not null then update public.carts set status='converted',updated_at=now() where id=c; end if; end;$$;
