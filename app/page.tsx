    ...
    <main className="min-h-screen overflow-hidden">
      <header className="mx-auto flex max-w-7xl items-center justify-between px-6 py-6 lg:px-8">
        ...
        <nav className="hidden items-center gap-6 text-sm font-semibold text-slate-600 md:flex">
          <Link href="/ai" className="hover:text-[#0b6b4f]">AI</Link>
          <Link href="/tools" className="hover:text-[#0b6b4f]">Tools</Link>
          <Link href="/farms" className="hover:text-[#0b6b4f]">Farms</Link>
          <Link href="/marketplace" className="hover:text-[#0b6b4f]">Marketplace</Link>
          <Link href="/blog" className="hover:text-[#0b6b4f]">Articles</Link>
        </nav>
      ...
      <section className="mx-auto max-w-7xl px-6 pb-20 pt-12 lg:px-8 lg:pt-20">
        <div className="mx-auto max-w-4xl text-center">
          ...
          <form action="/search" method="get" className="mx-auto mt-9 flex max-w-2xl items-center rounded-2xl border border-slate-200 bg-white p-2 shadow-xl shadow-emerald-900/5">
            <Search className="ml-3 text-slate-400" size={21} />
            <input name="q" className="min-w-0 flex-1 bg-transparent px-3 py-3 text-sm outline-none" placeholder="Ask AGRIVA anything about agriculture…" aria-label="Search AGRIVA" />
            <button type="submit" className="rounded-xl bg-[#0b6b4f] px-5 py-3 text-sm font-bold text-white">Search</button>
          </form>
        </div>

        <div id="features" className="mt-20 grid gap-5 md:grid-cols-2 lg:grid-cols-4">
          {modules.map(({ icon: Icon, title, text, href }) => (
            <Link href={href} key={title} className="group rounded-3xl border border-white bg-white p-6 shadow-lg shadow-slate-900/5 transition hover:-translate-y-1 hover:shadow-xl">
              <div className="mb-5 grid h-12 w-12 place-items-center rounded-2xl bg-emerald-50 text-[#0b6b4f]"><Icon size={23} /></div>
              <h2 className="font-extrabold text-[#12352a]">{title}</h2><p className="mt-2 text-sm leading-6 text-slate-500">{text}</p>
              <span className="mt-4 inline-flex items-center gap-1 text-xs font-bold text-[#0b6b4f]">Explore <ArrowRight size={14} className="transition group-hover:translate-x-1" /></span>
            </Link>
          ))}
        </div>

        <section className="mt-10">
          <div className="mb-5 flex items-end justify-between"><div><p className="text-xs font-bold uppercase tracking-widest text-[#0b6b4f]">Explore AGRIVA</p><h2 className="mt-2 text-2xl font-black text-[#12352a]">Everything farmers need, in one place.</h2></div></div>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {quickLinks.map(({ icon: Icon, title, text, href }) => <Link href={href} key={title} className="rounded-2xl border border-slate-200 bg-white p-5 transition hover:border-emerald-200 hover:shadow-md"><Icon className="text-[#0b6b4f]" size={21} /><h3 className="mt-3 font-bold text-[#12352a]">{title}</h3><p className="mt-1 text-sm text-slate-500">{text}</p></Link>)}
          </div>
        </section>

        <section id="intelligence" className="mt-10 grid gap-6 rounded-[2rem] bg-[#064b39] p-8 text-white md:p-10 lg:grid-cols-[1.1fr_.9fr]">
          <div><div className="mb-4 flex items-center gap-2 text-sm font-bold text-emerald-200"><TrendingUp size={17} /> AGRIVA Intelligence</div><h2 className="text-3xl font-black md:text-4xl">Search today. Discover tomorrow&apos;s demand.</h2><p className="mt-4 max-w-xl leading-7 text-emerald-50/75">AGRIVA is designed to learn from aggregated search demand and surface useful content and product opportunities.</p><Link href="/admin" className="mt-7 inline-flex items-center gap-2 rounded-xl bg-white px-5 py-3 text-sm font-bold text-[#064b39]">Open intelligence <ArrowRight size={16} /></Link></div>
          <div className="rounded-3xl bg-white/10 p-5 backdrop-blur"><div className="text-xs font-bold uppercase tracking-widest text-emerald-200">Demand Center</div><div className="mt-5 space-y-3">{["Rice disease management", "Organic fertilizer", "Tomato pest control"].map((q, i) => <div key={q} className="flex items-center justify-between rounded-2xl bg-white/10 p-4"><span className="text-sm font-semibold">{q}</span><span className="text-xs font-bold text-emerald-200">{184 - i * 42} searches</span></div>)}</div></div>
        </section>
